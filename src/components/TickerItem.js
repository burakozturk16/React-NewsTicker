...
import getPosition from './TickerHelper'


function getStartOffset({ offset, rect, direction, width }) {
    if (offset === 'run-in') {
        switch (direction) {
            case 'toRight':
                return -rect.width;
...
function getNextOffset({ from, rect, direction }) {
    switch (direction) {
        case 'toRight': {
            return from
        }
...
function shouldNextTriggerOnMount({ rect, mode, prevOffset, position, direction, width }) {
